...
bool InitD3D()
{
	...
	D3D12_INPUT_ELEMENT_DESC inputLayout[] =
	{
		...
		{ "TEXCOORDS", 0, DXGI_FORMAT_R32G32_FLOAT,0, 12, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 }
	};
	...
	m_model->LoadMesh("walk.FBX", positions, normals, texCoords, indices, numVertices, numIndices);

	Vertex* vertices = new Vertex[positions.size()];
	const UINT vertexBufferSize = sizeof(Vertex) * positions.size();
	for (size_t i = 0; i < positions.size(); i++)
	{
		...
		vertex.normal = XMFLOAT3(norm.x, norm.y, norm.z);
	...
	vertexData.pData = &vertices[0]; // pointer to our vertex array
	...
	const UINT indexBufferSize = sizeof(unsigned int) * indices.size();
	...
	indexData.pData = &indices[0]; // pointer to our index array
